      ...
      middleware = function expressRedisCache_Middleware (req, res, next) {
        ...
        self.get(name, domain.bind(function (error, cache) {
          ...
          if (cache.length && cache[0].body != null) {
            ...
            try {
              headersToRemove = res.getHeaders(); // Node v7.7.0+
            } catch (err) {
              headersToRemove = res._headers; // Pre node v7.7.0 [deprecated since v12.0.0]
            }

            Object.keys(headersToRemove).forEach((key) => {
          ...
          /** otherwise, cache request **/
            ...
            res.send = function (body) {
              ...
              Object.keys(currentHeaders).forEach(key => {
                headers[key] = currentHeaders[key];
              });
